...
// v17: GIF 띠 예산을 **띠 선택 코드를 그대로 돌려 나온 띠 폭**에서 잡는다(gifBudgetsOf, 좌우 여백 12px).
//      글자 행 여백으로 센 폭(360px)과 실제 띠(313px)가 달라 글자가 양 끝에 닿았다(실측 13 「大头爆震」).
// v18: 예산 한 글자 0.95em·좌우 여백 28px (koTextWidth 와 같은 기준). 0.85em·12px 로는 띠 313px 에
//      17자가 들어가 글자가 양 끝에 닿았다(실측 13 「大头爆震」).
// v19: 글자 폭 모델을 글자 종류별로(한글 0.85em·숫자/영문 0.5em·기호 0.35em·공백 0.3em) — 전부
//      1.0em 으로 세면 "360° 밀착핏" 예산이 5자로 떨어져 승인 문구가 잘린다. 예산 나눗수 0.8em.
const PROMPT_V = 19; // v8: 띠 프롬프트에 세로쓰기 유지 지시 복원 (2026-09-02) // v6: GIF 글자 띠 전용 프롬프트 분리 (2026-09-01) — 띠 crop 에도
...
// v17: GIF 제품 무결성 심사 결과에서 글자만 문제 삼은 항목(남은 중국어 등)을 거른다 — 요청문에
//      무시하라고 적어도 실측 12·13·14 세 번 다 hard 로 냈다(규칙 4). 잔류 관문이 따로 잡는다.
const VERIFY_V = 17;
...
// 22: 작아진 띠의 재시도는 같은 배율로(배율을 올리면 더 작아졌다: 84%→78%), 힌트에 "지금 Npx →
