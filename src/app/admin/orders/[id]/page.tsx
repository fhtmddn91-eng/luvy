...
export default async function AdminOrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
 ...
 <Link href="/admin/orders" className="text-[13px] text-muted hover:text-ink-deep">← 주문 목록</Link>
 ...
 <button type="submit" className="h-11 w-full bg-ink-deep text-[12px] font-bold uppercase tracking-[0.12em] text-white transition-opacity hover:opacity-80">
 변경 저장
 </button>
 </form>
 {order.status !== "CANCELED" && (
 <form action={cancelOrderPayment.bind(null, order.id)} className="mt-3 border-t border-hairline pt-3">
 <button type="submit" className="h-10 w-full border border-hairline bg-white text-[13px] font-bold text-ink-soft hover:border-ink-deep hover:text-ink-deep">
